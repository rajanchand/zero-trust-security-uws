        ...
        <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-6 text-left space-y-4">
          ...
          {signals && (
            <div className="border-t border-border pt-3 space-y-1 text-xs text-muted-foreground">
              <div>IP: <span className="text-foreground">{signals.ip}</span></div>
              <div>Location: <span className="text-foreground">{signals.city}, {signals.country}</span></div>
              <div>ISP: <span className="text-foreground">{signals.isp}</span></div>
              <div>Browser: <span className="text-foreground">{signals.browser} / {signals.os}</span></div>
              <div>Time: <span className="text-foreground">{new Date(signals.loginTime).toLocaleString()}</span></div>
            </div>
          )}
        </div>
